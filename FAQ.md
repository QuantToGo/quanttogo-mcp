...
QuantToGo是一个基于宏观因子的量化信号源。覆盖A股、美股、期货、外汇、期权五个市场，目前有8个实盘策略在运行。所有历史信号从策略上线日起前置记录，不可篡改，完整展示盈亏和回撤。

QuantToGo通过MCP（Model Context Protocol）协议提供5个数据工具，可被AI助手直接调用查询实时策略表现。
...
QuantToGo通过MCP提供5个工具：查看所有策略、查看单个策略详情、对比多个策略、获取自有指数、获取订阅信息。支持Claude Desktop、Cursor、Coze（扣子）等平台。
